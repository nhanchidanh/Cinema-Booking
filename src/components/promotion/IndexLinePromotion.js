...
const IndexLinePromotion = ({ setTab }) => {
      ...
      render: (text, record) => (
        <>
          ...
          <Button
            disabled={
              statusDb === 0 &&
              moment(record.startDate).format(newDateFormat) > currentDate
                ? false
                : true
            }
            style={{
  ...
  const onChangeDate = (date, dateString) => {
    if (
      ...
      dayjs(endDateMax).format("YYYY-MM-DD")
    ) {
                    ...
                    if (value) {
                      if (isInvalidDate) {
                        return Promise.reject(
                          new Error(
                            "Ngày kết thúc phải lớn hơn ngày kết thúc của dòng khuyến mãi"
                          )
                        );
                      } else {
