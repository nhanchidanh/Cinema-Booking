    ...
    await Promise.all(
      data.map(async (order) => {
        const movie = await MovieRepository.getMovieById(
          order.ShowMovie.Show.Movie.id
        );
        const end_time = moment(order.ShowMovie.ShowTime.showTime, "HH:mm")
          .add(movie.duration, "minutes")
          .format("HH:mm");
        order.dataValues.ShowMovie.dataValues.ShowTime.dataValues.end_time =
          end_time;
      })
    );
    return data;
  ...
  async create(order) {
    ...
    try {
      await sequelize.transaction(async (t) => {
        ...
        const orderDetail = await OrderDetailRepository.bulkCreate(
          payloadOrderDetail,
          { transaction: t }
        );
        ...
        if (order.idCustomer !== 36) {
          ...
          const newPoint =
            ...
            Number(orderDetail.length);
          await MemberShipRepository.updatePoint(order.idCustomer, newPoint, {
            transaction: t,
          });
        ...
        if (order.product_sp && order.product_sp.length > 0) {
          ...
          await OrderDetailRepository.bulkCreate(payloadOrderDetailSp, {
            transaction: t,
          });
        }
        ...
        if (order.promotionApplicalbe && order.promotionApplicalbe.length > 0) {
          for (let i = 0; i < order.promotionApplicalbe.length; i++) {
            ...
            await PromotionResultRepository.create(payloadPromotion, {
              transaction: t,
            });
            // }
          }
  ...
  async refund(id, body) {
    try {
      await sequelize.transaction(async (t) => {
        ...
        const orderDetailTypeSeat = await OrderDetailRepository.getByType(
          id,
          1
        );
        ...
        await ReservationService.cancel({
          showTime_id: order.ShowMovie.id,
          seats: seats,
        });
        // update status promotion result
        await PromotionResultRepository.updateStatusByOrderId(id, 0, {
          transaction: t,
        });
        // update status order
