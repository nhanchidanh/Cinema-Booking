...
class CinemaService {
  async getAllCinema(query) {
    ...
    await Promise.all(
      data.map(async (cinema) => {
        const countCinemaHall = await CinemaHallRepository.countHallByCinemaId(
          cinema.id
        );
  ...
  async createCinema(cinema) {
    ...
    const nameIsExist = await CinemaRepository.getCinemaByName(cinema.name);
    if (nameIsExist) {
      throw new Error("Rạp đã tồn tại");
    }
